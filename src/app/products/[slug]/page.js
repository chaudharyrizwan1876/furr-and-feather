...
export default function ProductDetailPage({ params }) {
  ...
  const getActivePrice = () => selectedVariant ? selectedVariant.price : product.price;
  const getActiveStock = () => selectedVariant ? selectedVariant.stock : product.stock;

  const handleAddToCart = () => {
    const cartProduct = selectedVariant
      ? { ...product, price: selectedVariant.price, stock: selectedVariant.stock, name: `${product.name} (${selectedVariant.label})` }
      : product;
          ...
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '20px', flexWrap: 'wrap' }}>
              ...
              {!selectedVariant && hasDiscount && (
                <span style={{ fontSize: '1rem', color: 'var(--text-muted)', textDecoration: 'line-through' }}>Rs. {product.discountPrice.toLocaleString()}</span>
              )}
              {!selectedVariant && hasDiscount && (

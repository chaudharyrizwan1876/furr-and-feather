...
export default function AddProductPage() {
  ...
  const [form, setForm] = useState({
    ...
    sku: '', stock: '', unit: 'Box', suitableFor: '', shortDescription: '', description: '',
  });
  ...
  const handleSubmit = async (status) => {
    ...
    try {
      ...
      if (!productRes.ok) {
        throw new Error(productData.message || 'Could not create product');
      }
  ...
  return (
    <div>
      <h1 style={{ fontSize: '1.6rem', fontWeight: '800', color: 'var(--text)', marginBottom: '20px' }}>Add New Product</h1>
      ...
      <div style={{ backgroundColor: 'white', borderRadius: '14px', padding: '28px', boxShadow: '0 2px 10px rgba(0,0,0,0.06)' }}>
          ...
          {!imagePreview ? (
            ...
            <div style={{ position: 'relative', width: '160px' }}>
              <div style={{ backgroundColor: 'var(--bg)', borderRadius: '10px', padding: '8px', border: '2px solid var(--border)' }}>
                <img src={imagePreview} alt="Preview" style={{ width: '100%', height: '140px', objectFit: 'cover', borderRadius: '6px' }} />
              </div>
        ...
        <div style={{ display: 'flex', gap: '12px' }}>
          <button onClick={() => router.push('/admin/products')} disabled={loading}
